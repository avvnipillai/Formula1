...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .track-shadow{position:absolute;bottom:28px;left:0;width:100%;height:16px;
    background:linear-gradient(180deg,transparent,rgba(168,200,232,.03));}
  ...
  .streak:nth-child(1){width:140px;height:2px;bottom:36px;
    background:linear-gradient(90deg,transparent,rgba(168,200,232,.35));animation-delay:0s;}
  ...
  /* PODIUM — only 2 cards now */
  .podium-row{display:grid;grid-template-columns:1fr 1fr;gap:1px;
  ...
  .p-detail{font-size:11px;margin-top:7px;font-weight:500;font-family:'Barlow Condensed',sans-serif;letter-spacing:.5px;}
  ...
  .tab.active::after{content:'';position:absolute;bottom:-1px;left:0;right:0;height:2px;
    background:linear-gradient(90deg,transparent,var(--steel),transparent);filter:blur(3px);}
  ...
  .stat-value.steel{color:var(--steel);text-shadow:0 0 16px rgba(168,200,232,.3);}
  ...
  .metric-val{font-family:'Orbitron',monospace;font-size:26px;font-weight:700;color:var(--steel);
    text-shadow:0 0 20px rgba(168,200,232,.3);}
  ...
  .sec{font-family:'Barlow Condensed',sans-serif;font-size:11px;letter-spacing:3px;
    text-transform:uppercase;color:var(--steel);margin-bottom:12px;padding-bottom:7px;
    border-bottom:1px solid var(--border);display:flex;align-items:center;gap:8px;}
  .sec::before{content:'';display:inline-block;width:3px;height:12px;
    background:var(--steel);border-radius:2px;flex-shrink:0;}
  ...
  tbody tr:hover td{background:rgba(168,200,232,.03);}
  .badge{display:inline-block;padding:2px 9px;border-radius:20px;font-size:10px;font-weight:700;letter-spacing:.5px;}
  .badge.yes{background:rgba(225,6,0,.15);color:#ff7070;border:1px solid rgba(225,6,0,.25);}
  ...
  .cbadge.medium{background:rgba(232,232,232,.08);color:#d8d8d8;border:1px solid rgba(232,232,232,.2);}

  .loader{text-align:center;padding:60px;color:var(--muted);font-family:'Orbitron',monospace;
    font-size:10px;letter-spacing:3px;text-transform:uppercase;
    animation:pulse 1.5s ease-in-out infinite;}
  @keyframes pulse{0%,100%{opacity:.4}50%{opacity:1}}
  .err{color:#ff7070;padding:20px;font-size:13px;background:rgba(225,6,0,.06);
  ...
  <div class="hero-track">
    ...
    <div class="car-wrap">
      <svg width="210" height="54" viewBox="0 0 210 54" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <rect x="92" y="8" width="18" height="5" rx="2" fill="rgba(168,200,232,.15)"/>
        ...
        <rect x="22" y="19" width="155" height="2" rx="1" fill="rgba(168,200,232,.2)"/>
        ...
        <circle cx="34"  cy="38" r="12" fill="none" stroke="rgba(168,200,232,.07)" stroke-width="2"/>
        <circle cx="165" cy="38" r="12" fill="none" stroke="rgba(168,200,232,.07)" stroke-width="2"/>
      </svg>
    </div>
  </div>

  <!-- 2 podium cards only: winner + 2nd -->
...
def api_regression():
    ...
    # enforce: compound filter only meaningful for All Drivers
    if driver_filter:
        compound_filter = ""
    ...
    try:
        ...
        def row_compound(r):
            for cmp in ['SOFT', 'MEDIUM', 'HARD']:
                col = f"Compound_{cmp}"
                if col in r.index and str(r[col]).upper() in ('TRUE', '1', 'YES'):
                    return cmp
            return "UNK"
        ...
        # line plot — use soft steel blue for actual line
